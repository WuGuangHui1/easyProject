...
public class UserConfigurationTest extends BaseTest {
	// 用户与身份模块 用户配置  （注释：该功能项用例已完成！整体进行验证……
	@BeforeClass
	public void openUrl() {
	...
	// 用户配置 - 微信绑定
	@Test(description = "正确显示‘微信绑定’界面", priority = 0)
	...
	@Test(description = "解绑-操作", dependsOnMethods = "test_wechatInfo", priority = 1)
	...
	// 用户配置 - 基础配置
	@Test(description = "正确显示‘用户配置’界面", priority = 2)
	public void test_userBasicConfigu() throws InterruptedException {
		...
		assertTrue(webtest.isTextPresent("注册配置"), "页面显示失败");
	}

	@Test(description = "注册配置", dependsOnMethods = "test_userBasicConfigu", priority = 3)
...
//		WebElement target = webtest.getElementByXpath("//*[@id='nestable_list_2']");
	...
	@Test(description = "登录配置", dependsOnMethods = "test_userBasicConfigu", priority = 4)
	...
	@Test(description = "邮箱验证模版", dependsOnMethods = "test_userBasicConfigu", priority = 5)
	...
	@Test(description = "邮箱激活模版", dependsOnMethods = "test_userBasicConfigu", priority = 6)
	...
	@Test(description = "短信配置", dependsOnMethods = "test_userBasicConfigu", priority = 7)
	...
	@Test(description = "基础配置", dependsOnMethods = "test_userBasicConfigu", priority = 8)
	...
	// 用户配置 - 权限组管理
	@Test(description = "正确显示‘权限管理’信息界面", priority = 9)
	...
	@Test(description = "新增权限管理项", dependsOnMethods = "test_authorityManage", priority = 10)
