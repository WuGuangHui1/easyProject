...
public class TitleManagementTest extends BaseTest {
	// 用户与身份模块 头衔管理 （注释：该功能项用例已完成！整体进行验证……
	@BeforeClass
	public void openUrl() {
...
//			action.moveToElement(subMenu).click().perform();滑动到运营功能项，所以不能采用这种方式
		subMenu.click();
		Thread.sleep(3000);
	...
	public void test_relatedTitle() throws InterruptedException {
		...
		if (texts.contains("成功")) {
			Assert.assertEquals(texts, "添加关联成功，页面即将自动跳转~");
		} else {
			while (texts.contains("已经拥有该头衔")) {
				int num = driver.findElements(By.xpath(
						"/html/body/div[1]/div/div/section[2]/div/div[2]/div/div[3]/div/div/form/div[5]/select/option"))
						.size();
				// System.out.println("num："+num);
				webtest.click(
						"xpath=/html/body/div[1]/div/div/section[2]/div/div[2]/div/div[3]/div/div/form/div[5]/select");
				Thread.sleep(1000);
				// 选择下拉框中下一个选项
				webtest.click(
						"xpath=/html/body/div[1]/div/div/section[2]/div/div[2]/div/div[3]/div/div/form/div[5]/select/option["
								+ i + "]");
				webtest.click("xpath=//*[@id='submit']");
				Thread.sleep(1000);
				driver.switchTo().alert().accept();
				...
				String texts2 = content2.getText();
				Thread.sleep(2000);
				System.out.println(texts2);
				if (texts2.contains("成功")) {
					Assert.assertEquals(texts2, "添加关联成功 页面即将自动跳转~");
					break;
				}
				if (i <= num) {
					i++;
...
//		Thread.sleep(3000);
		...
		action.moveToElement(parentMenu).perform();
		Thread.sleep(3000);
//		WebElement subMenu = webtest.getElementByText("待审核用户头衔");
...
//		subMenu.click();
	...
	public void test_applyTitle() throws InterruptedException {
		...
		action.moveToElement(parentMenu).perform();
		Thread.sleep(3000);
//		WebElement subMenu = webtest.getElementByText("被驳回的头衔申请");
		WebElement subMenu = webtest
				.getElementByXpath("/html/body/header/nav/div/ul[1]/li[2]/ul/li/div/div/div[2]/ul/li[4]/a");
		subMenu.click();
		Thread.sleep(3000);
...
//		WebElement parentMenu = webtest.getElementByXpath("/html/body/header/nav/div/ul[1]/li[2]/a");
		...
		action.moveToElement(parentMenu).perform();
		Thread.sleep(3000);
//		WebElement subMenu = webtest.getElementByText("头衔列表");
		WebElement subMenu = webtest
	...
	public void test_addTitle() throws InterruptedException, AWTException {
		...
		Thread.sleep(2000);
		Robot r = new Robot();
		r.keyPress(KeyEvent.VK_ENTER);// 摁下enter键
		r.keyRelease(KeyEvent.VK_ENTER);// 放开enter键
		Thread.sleep(2500);
		...
		Thread.sleep(1500);
		// 选择颜色
//		webtest.click("xpath=//*[@id='9900cc']");//id定位不到不在视野内，换绝对路径
		webtest.click(
